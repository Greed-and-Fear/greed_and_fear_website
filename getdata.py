...
stock_df = pd.DataFrame(columns=["Stock","Date","Open","Close","High","Low","Volume","color","Shade-Per"])
...
for each_date in date_list:
    ...
    col,shaded_percentage = get_candle_details(open,close,high,low)
    stock_df.loc[len(stock_df.index)] = [symbol,each_date,open,close,high,low,float(present_stock_data['5. volume']),col,shaded_percentage] #type: ignore
